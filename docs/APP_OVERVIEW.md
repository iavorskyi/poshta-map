...
Реєстр юр.осіб на дільниці: назва, адреса (вільний текст), опис, бул. **«забирають пошту самі»** (інакше показуємо бейдж «Носимо пошту»). До кожної організації — список контактних осіб (`OrganizationContact`): імʼя, телефон, примітка.

### 12.1. Пошук
Один input — шукає по назві, адресі, опису, іменах контактів і телефонах одночасно.
- Текстові поля: спершу `includes` на нормалізованому рядку (lowercase, єдиний апостроф, ё→е, без пунктуації), далі per-token Levenshtein через `tokenizeName/levenshtein` з [nameMatch.ts](src/lib/nameMatch.ts) — допуск 0/1/2/3/4 залежно від довжини токена; найменша відстань → менший `score` → вище в списку
