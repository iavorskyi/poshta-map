...
### Organization — юридична особа на дільниці
- `name`, `address?`, `description?`, `picksUpMail: Boolean @default(false)` (true = забирають свою пошту самі, не носимо)
- `storageLocation?` — де лежить пошта для цієї організації (коробка, шухляда, а/с — вільний текст). Шукається разом з адресою/описом
- `messageTemplate?` — шаблон повідомлення для копіювання (токен `{організація}`), спільний для всіх контактів; snapshot глобального дефолту при створенні (див. 12.2b)
- `contacts[]` → `OrganizationContact`
- Окремо від `Subscriber`: тут не про передплати газет, а про загальні контактні дані організації
- Індекс: `name`

### OrganizationContact — контактна особа в організації
- `organizationId` (Cascade), `name`, `phone?`, `note?`
- Індекс: `organizationId`

### OrganizationRelation — симетричний звʼязок між двома організаціями
- `aId`, `bId`, `note?`; обидва FK мають `onDelete: Cascade`
- Інваріант: завжди `aId < bId` (підтримує `orderedPair` у [actions.ts](src/app/(authed)/organizations/actions.ts)) — один рядок на пару
- Unique `[aId, bId]`, індекси `aId`, `bId`

### AppSetting — глобальні key-value налаштування
- `key @id`, `value`, `updatedAt`. Наразі один ключ: `orgMessageTemplateDefault` — дефолтний шаблон повідомлення для нових організацій (див. 12.2b)
...
- `removePensionerFromRound(roundId, pensionerId)` — відвʼязує всі виплати пенсіонера від обходу (`roundId = null`, `roundPosition = 0`). Виплати НЕ видаляються — лишаються в реєстрі `/current-payments`. Раніше робився `deleteMany`, що знищував дані (зокрема щойно імпортовані виплати, які адмін випадково клав у не той обхід)
...
### 11.9. Імпорт передплат з XLSX (двокроковий)
Кнопка **«Імпорт з Excel»** у шапці `/subscriptions/publications` (лише admin, `canManageSubscriptions`).
- [ImportSubscriptions.tsx](src/app/(authed)/subscriptions/ImportSubscriptions.tsx) — модал завантаження: посилання на шаблон + вибір файлу → `previewSubscriptionsImport`
- [SubscriptionImportPreview.tsx](src/app/(authed)/subscriptions/SubscriptionImportPreview.tsx) — покрокове підтвердження рішень → `applySubscriptionsImport`
- [importActions.ts](src/app/(authed)/subscriptions/importActions.ts) — server actions preview/apply
- [src/lib/subscriptionImport.ts](src/lib/subscriptionImport.ts) — `parseSubscriptionsXlsx` (парсер аркуша «Передплати» + довідник «Видання» code→name) і `buildSubscriptionsTemplate` (генерація шаблону з випадайками кодів/типу/доставки, валідаціями місяців, формулами-помічниками VLOOKUP назви й SUM «Всього»)
- [api/subscriptions/template/route.ts](src/app/api/subscriptions/template/route.ts) — `subscriptions-template.xlsx` з наявними виданнями як довідником

Формат аркуша «Передплати»: один рядок = одна передплата `(передплатник × видання × рік)`. Колонки Рік, Код видання, ПІБ, Тип, Телефон, Вулиця/Буд./Кор./Кв., Доставка, `01…12` (кількість примірників на місяць), Всього (авто). Логіка:
- **Видання** беруться за кодом; відсутні авто-створюються (`upsert` за `code`, назва з довідника «Видання»)
- **Передплатник** розпізнається як `exact` (ПІБ-токени + адреса збігаються), інакше `fuzzy` (broad-search `findClosestPensionersByName` по всій базі, `distance ≤ 4`), інакше `none`. Рішення (`use_existing`/`create_new`/`skip`) підтверджується вручну — нові передплатники створюються **лише за явним підтвердженням**
- **Адреса** резолвиться через `findBuildingByAddress`: `exact`/`loose` → `buildingId`; інакше зберігається як `streetText/numberText` (off-district валідно). `PICKUP` без адреси; для `ADDRESS` без вулиці/будинку рядок не можна створити як нового — лише привʼязати до наявного або пропустити
- **Ідемпотентність**: підписка `upsert` за `[subscriberId, publicationId, year]` — повторний імпорт оновлює `activeMonths`, дублів не створює. Рядки з усіма нулями пропускаються

---

## 12. Організації `/organizations`

[page.tsx](src/app/(authed)/organizations/page.tsx), [OrganizationsClient.tsx](src/app/(authed)/organizations/OrganizationsClient.tsx), [\[id\]/page.tsx](src/app/(authed)/organizations/[id]/page.tsx), [\[id\]/OrganizationDetailClient.tsx](src/app/(authed)/organizations/[id]/OrganizationDetailClient.tsx), [actions.ts](src/app/(authed)/organizations/actions.ts)

Реєстр юр.осіб на дільниці: назва, адреса (вільний текст), опис, бул. **«забирають пошту самі»** (інакше показуємо бейдж «Носимо пошту»), `storageLocation` — де шукати листи для цієї орг (коробка/шухляда/а/с). До кожної організації — список контактних осіб (`OrganizationContact`): імʼя, телефон, примітка.

### 12.1. Пошук
Один input — шукає по назві, адресі, опису, іменах контактів і телефонах одночасно.
- Текстові поля (назва, адреса, опис, локація комірки): спершу `includes` на нормалізованому рядку (lowercase, єдиний апостроф, ё→е, без пунктуації), далі per-token Levenshtein через `tokenizeName/levenshtein` з [nameMatch.ts](src/lib/nameMatch.ts) — допуск 0/1/2/3/4 залежно від довжини токена; найменша відстань → менший `score` → вище в списку
- Телефон: коли в запиті ≥3 цифри, порівнюємо лише цифри (subscring), бо в БД формати різні («+38…», «066…»)
- Бейдж «збіг: адреса/телефон контакту/контакт» поряд з тими хітами, де матч стався не по назві
- Усе ранжується в памʼяті (`searchOrganizations` у [src/lib/orgSearch.ts](src/lib/orgSearch.ts)) — записів десятки, SQL-fuzzy не потрібен

### 12.2. CRUD
- `createOrganization`, `updateOrganization`, `deleteOrganization` — admin (`canManageOrganizations`)
- `addContact`, `updateContact`, `deleteContact` — admin; видалення організації каскадом видаляє контакти (`onDelete: Cascade`)
- Інлайн-форми, без модалок: на списку — створення; на деталі — редагування організації та додавання/редагування контактів

### 12.2a. Швидкі переходи в месенджери
- Біля `tel:`-лінка кожного контакту (деталь, прев'ю списку, звʼязані орг) — кнопки **Viber** і **Telegram** ([MessengerButtons.tsx](src/components/MessengerButtons.tsx), deep-links у [src/lib/messengerLinks.ts](src/lib/messengerLinks.ts))
- Viber: `viber://chat?number=…`; Telegram: `https://t.me/+…`. Номер нормалізується до цифр (укр. `0XXXXXXXXX` → `380…`)
- Наявність акаунта в месенджері **не перевіряється** (публічного API немає) — кнопки показуються завжди, реєстрацію абонента визначає сам застосунок при відкритті
- `variant="card"` (кольорові пігулки на деталі) / `variant="compact"` (маленькі іконки в прев'ю, з `stopPropagation`, щоб не тригерити перехід у картку)

### 12.2b. Шаблон повідомлення + кнопка копіювання
Deep-links не вміють одночасно і націлитись на номер, і підставити текст, тому текст **копіюється** окремою кнопкою поряд з іконками месенджерів (усюди: список орг, прев'ю контактів, картка контакту, звʼязані орг).
- `Organization.messageTemplate?` — шаблон, спільний для всіх контактів організації. Токен `{організація}` підставляється назвою ([renderTemplate](src/lib/messengerLinks.ts)); готовий текст рахується на сервері й передається у `MessengerButtons` як `copyText`
- При створенні організації `messageTemplate` = **snapshot** глобального дефолту (або власний текст, якщо адмін ввів). Далі редагується у формі організації
- Наявні орги без свого `messageTemplate` (null) → фолбек на глобальний дефолт при рендері
- **Глобальний дефолт** зберігається в `AppSetting` (key `orgMessageTemplateDefault`), редагується у admin-панелі зверху списку орг (`setOrgMessageTemplateDefault`); якщо в БД порожньо — хардкод `DEFAULT_ORG_MESSAGE_TEMPLATE`
- Кнопка копіювання видима всім (не лише admin); тост «Текст скопійовано»

### 12.3. Звʼязки між організаціями
Кейс: якщо по основній організації не дозвонитись, можна швидко знайти запасні контакти у звʼязаній (філія, спільний керівник тощо).

- Симетричний звʼязок — `OrganizationRelation` (`aId`, `bId`, optional `note`). Інваріант: у БД завжди `aId < bId` (підтримує `orderedPair` в [actions.ts](src/app/(authed)/organizations/actions.ts)), тому unique-ключ `[aId, bId]` працює в обидва боки і немає дублів
- Каскадне видалення з обох боків (`onDelete: Cascade`) — зник звʼязок при видаленні будь-якої з двох орг
- На сторінці деталі — секція «Повʼязані організації»: ім'я → лінк, бейдж самовивозу, до 4 контактів кожної звʼязаної (telephone-лінки), необовʼязкова примітка про природу звʼязку
- Пікер `+ Звʼязати`: інлайн пошук тим самим `searchOrganizations`, показує до 8 кандидатів (виключно ті, хто ще не звʼязаний і не сама орг); клік «Звʼязати» створює запис без примітки, потім її можна додати/редагувати
- `linkOrganization` робить upsert, `unlinkOrganization` видаляє за `relationId`, `updateRelationNote` оновлює лише примітку

---

## 13. Спільні компоненти
...
## 16. Бізнес-нюанси та реальні ребра
...
29. **Імпорт передплат — двокроковий** (за зразком імпорту виплат): preview з розпізнаванням передплатника (exact ПІБ+адреса / fuzzy broad-search / none) та видання за кодом, потім apply за підтвердженими рішеннями. Видання авто-створюються за кодом, підписка ідемпотентна за `(subscriberId, publicationId, year)`; нові передплатники — лише за явним підтвердженням.

---

## 17. Деплой / середовище
