...
- `activeMonths: Boolean[]` — масив на 12 елементів (Січ-Гру), `true` = активна цього місяця
- `notes?`, `createdAt`, `updatedAt`
...
- **Auto-create пенсіонерів**, якщо за адресою + ФІО не знайдено (комміт `0006cb6`)
- Auto-create будинків через нормалізацію вулиць ([src/lib/streetMatch.ts](src/lib/streetMatch.ts))
- Дедуп за `(pensionerId, paymentId, year, month, day)` — повтори в файлі потрапляють у `warnings`
...
- **Пропозиції**: пенсіонери з `pensionPaymentDay === день обраної дати`, **за вирахуванням уже доданих та оплачених**
- При додаванні пенсіонера підтягуються його шаблонні типи виплат **за останні 6 місяців** ([src/app/(authed)/rounds/actions.ts](src/app/(authed)/rounds/actions.ts))
- Уже наявні неоплачені виплати поточного місяця **підтягуються в чернетку з `existingId`**, оплачені — приходять заблокованими з підписом «оплачено»
...
Матриця **передплатники × 12 місяців** для обраного року (`?year=`). Кожна клітинка — чекбокс `activeMonths[i]`, на зміну викликається `upsertSubscription`. Якщо запису `Subscription` для пари передплатник/видання/рік ще немає, він створюється з нульовим масивом і вмикається лише потрібний місяць. YearSwitcher — посилання на попередній/наступний рік.
...
Нижче — 12-місячний редактор по виданнях за обраний рік (`useOptimistic` для миттєвого toggle), кнопка «Додати видання» з фільтром уже доданих публікацій. YearSwitcher як на видавництві.
...
- `upsertSubscription({ subscriberId, publicationId, year, monthIndex, active })` — створює рядок з нульовим масивом якщо немає, інакше оновлює один індекс
...
| [scripts/import-subscriptions-2026.ts](scripts/import-subscriptions-2026.ts) | Одноразовий ідемпотентний імпорт передплат на 2026 рік: 48 видань, 86 передплатників, 107 підписок. Використовує `findBuildingByAddress` для авто-прив'язки до будинків дільниці; off-district адреси зберігає як `streetText/numberText` |

---

## 15. Бізнес-нюанси та реальні ребра

1. **Дашборд показує тільки моїх пенсіонерів** (`pensioner.postmanId === me.id`), включно з прострочкою — навіть якщо фактично обхід робив інший листоноша.
...
10. **Пропозиції в «Новий обхід»** базуються на `pensionPaymentDay === day(date)` мінус ті, що вже мають оплачені виплати на цей день.
11. **Дедуп при імпорті виплат** на парі `(pensionerId, день+місяць+рік)` для одного `paymentId`.
12. **Auto-create пенсіонерів** під час імпорту виплат — якщо за `(нормалізована_вулиця, номер, ФІО)` не знайдено. Без квартири — двох однофамільців у будинку не розрізнити, другий оновить першого.
...
26. **Subscription.activeMonths** — `Boolean[12]`, де індекс 0 = січень. У БД зберігається масив (PostgreSQL `Boolean[]`); UI завжди нормалізує до 12 елементів.
